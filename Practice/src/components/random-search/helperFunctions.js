...
const API_KEY = "0f2c91ee1a0541deb41f94ac002694f6";
...
export const fetchRandomChoicesRecipes = async ({ choice }) => {
  ...
  switch (choice) {
    ...
    case "oven":
      endpoint =
        endpoint + "complexSearch?apiKey=" + API_KEY + "&equipment=" + choice;
      break;
    case "easy":
      endpoint =
        endpoint + "complexSearch?apiKey=" + API_KEY + "&maxReadyTime=" + 15;
      break;
    default:
  ...
  try {
    ...
    if (!randomRecipeData.recipes) {
      ...
      console.log(recipe);
      const responseRecipeFullInfo = await axios.get(
